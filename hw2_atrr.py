...
def main():
    ...
    while(True):
        choice = raw_input("Choices: ) \n"
                        "1: Veth pair Demo \n"
                        "2: Bridged network Demo \n"
                        "3: VXLAN Demo \n"
                        "4: GRE demo \n"
                        "5: Exit\n")
        if choice:
            if str(choice) == '1':
                print("Please provide containers (1 and 2) IP: \n")
                sc = "We demonstarte the containers with name as IP and connect them to each other via veth pair\n"
                print(sc)
                c1_name = raw_input(
                    "Enter the container1 IP: \n")
                c1_name = unicode(c1_name, "utf-8")
                try:
                    ip = ipaddress.ip_address(c1_name)
                except:
                    print('IP address is invalid: %s' % c1_name)
                    print('Usage :  5.5.5.5')
                    c1_name = ''
                ...
                if c1_name and c2_name:
                    connect_two_containers_with_veth(c1_name, c2_name)
                    print("Infrastructure created")
                else:
                    print("Input is invalid")

            if str(choice) == '2':
                sc = "Please provide the Bridge name and the Containers IPs (comma seperated)" \
                    " values\n"
                print(sc)
